...
interface ThreadHistoryProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  position?: "left" | "right";
  /** When true, ThreadHistory does not manage the global --sidebar-width CSS var. */
...
const ThreadHistory = React.forwardRef<HTMLDivElement, ThreadHistoryProps>(
  (
    ...
    // Update CSS variable when sidebar collapses/expands (unless external layout manages spacing)
    React.useEffect(() => {
    ...
    return (
      ...
      >
        <div
          ref={ref}
          className={cn(
            // Inline container when embedded in external sidebar: no borders/background of its own
            externalLayout
              ? "bg-transparent h-full relative transition-all duration-300"
              : "border-flat bg-container h-screen fixed top-0 transition-all duration-300",
            // Only draw side borders when we're the layout owner
            !externalLayout && (position === "left" ? "border-r left-0" : "border-l right-0"),
            // Only apply internal width toggling when NOT using an external layout
            externalLayout ? "w-full" : isCollapsed ? "w-12" : "w-64",
            className,
          )}
          {...props}
        >
...
/**
 * Button to open the Library page
 */
const ThreadHistoryLibraryButton = React.forwardRef<
  HTMLAnchorElement,
...
>((props, ref) => {
  ...
  return (
    <a
      ...
      href="/library"
      className={cn(
        "flex items-center gap-2 rounded-md mb-3 hover:bg-backdrop transition-colors cursor-pointer",
        isCollapsed ? "p-1 justify-center" : "p-2",
      )}
      {...props}
    >
...
ThreadHistoryLibraryButton.displayName = "ThreadHistory.LibraryButton";

/**
 * Button to go to Chat view
 */
const ThreadHistoryChatButton = React.forwardRef<
  HTMLAnchorElement,
  React.AnchorHTMLAttributes<HTMLAnchorElement>
>((props, ref) => {
  const { isCollapsed } = useThreadHistoryContext();
...
ThreadHistoryChatButton.displayName = "ThreadHistory.ChatButton";
